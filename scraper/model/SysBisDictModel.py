...
from utils.BaseSaasModel import BaseSaasModel
from utils.DataVo import SysConfigVO, ScraperCfgVo, PlatformOrderCfgVo
...
class SysBisDict(BaseSaasModel):
    ...
    @staticmethod
    def get_dict_by_dir(dict_dir,session=None, tenant_id=None):
        """
        ...
        if session is None:
            session =DBSession()
            should_close = True
        query = session.query(SysBisDict).filter(
